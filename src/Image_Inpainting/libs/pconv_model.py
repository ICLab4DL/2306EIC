...
class PConvUnet(object):
    ...
    def __init__(self, img_rows=512, img_cols=512, vgg_weights="imagenet", inference_only=False, net_name='default', gpus=1, vgg_device=None):
        ...
        vgg_weights : which weights to pass to the vgg network.
        inference_only (bool): initialize BN layers for inference.
        net_name : Name of this network (used in logging).
        vgg_device : Specify which device to run VGG inference on.
        e.g. if training on 1 GPUs, vgg inference could be running on the GPU which is also training the UNet.
        """
        ...
        # VGG layers to extract features from (first maxpooling layers, In Liu's original paper)
    ...
    def loss_total(self, mask):
        ...
        def loss(y_true, y_pred):
            ...
            # Return loss function, you can see it in Liu's original paper
    ...
    def loss_style(self, output, vgg_gt):
        """Style loss based on output/computation, used for both eq. 4 & 5 Liu's original paper"""
        loss = 0
    ...
    def l1(y_true, y_pred):
        ...
        if K.ndim(y_true) == 4:
            ...
            raise NotImplementedError("DONOT Calculating L1 loss on 1D tensors") # this the a error here, when implate the code, it hurts me
